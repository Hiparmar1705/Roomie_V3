...
export default function SavedScreen({ navigation }) {
  ...
  const loadSaved = useCallback(async () => {
    ...
    const [favoriteListingResponse, favoriteIdResponse] = await Promise.all([
      listingService.getFavoriteListings(user),
      listingService.getFavoriteIds(user),
    ]);
    setSavedListings(favoriteListingResponse);
    setFavoriteIds(favoriteIdResponse);
    // if a saved item disappears, we quietly drop it from compare selection too
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptyText}>Tap the heart on any listing to add it here for later.</Text>
          </View>
        }
      />
    </SafeAreaView>
